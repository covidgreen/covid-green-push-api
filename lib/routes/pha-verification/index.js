...
async function phaVerification(server, options, done) {
    ...
    const checkSession = async (token) => {
        ...
        try {
            data = (await axios({
                url: `https://cognito-idp.${process.env.AWS_REGION}.amazonaws.com/${process.env.COGNITO_POOL_ID}/.well-known/jwks.json`,
                json: true
            })).data;
    ...
    server.route({
        ...
        handler: async (request) => {
            ...
            // Push API POST request goes here.
            const config = await getConfig();

            try {
                const date = new Date();
                const month = date.getMonth() + 1;
                // TODO: This shoudl read from the body instead.
                const dateString = `${date.getFullYear()}-`
                    + `${month < 10 ? '0' + month : month}`
                    + `-${date.getDate()}`;

                // TODO: Validate and ensure phone is supplied.
                const phone = request.body.phone.replace(/[\-]+/g, '');
                // TODO: This should just now be invoked directly vs. HTTP.
                // const response = await axios({
                //     method: 'POST',
                //     url: `${config.pushApiHost}:${config.pushApiPort}/notify/positive`,
                //     headers: {
                //         'Authorization': `Bearer ${config.security.jwtSecret}`,
