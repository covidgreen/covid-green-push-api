...
const { addMinutes, format, subHours } = require('date-fns')
...
async function notify(server, options, done) {
  ...
  const handler = async (request, response) => {
    ...
    if (options.issueProxy.url !== '') {
      ...
      code = await generateVerificationCode(
        request.log,
        offsetSymptomDate,
        resolvedTestType
      )
      expiresAt = addMinutes(new Date(), options.sms.codeLifetime)
    }
    ...
    return {
      ...
      error: '',
      expiresAt,
      expiresAtTimestamp: format(expiresAt, 't'),
      smsSent,
      uuid
    }
  }
