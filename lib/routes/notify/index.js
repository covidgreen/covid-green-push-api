...
const { addMinutes, subHours, subDays, addDays, format } = require('date-fns')
...
async function notify(server, options, done) {
  ...
  const generateVerificationCode = async (log, onsetDate, testType, mobile) => {
    try {
      ...
      const longCode = options.sms.deeplinkAllowed
        ? cryptoRandomString({
            length: 16,
  ...
  const handler = async (request, response) => {
    ...
    const { jobId, mobile, phone, onsetDate, symptomDate, testDate, testType } =
      request.body
    ...
    if (resolvedSymptomDate) {
      const today = format(new Date(), 'yyyy-MM-dd')
      const startRange = subDays(new Date(today), 14)
      const endRange = addDays(new Date(today), 14)
      if (
        new Date(resolvedSymptomDate) < startRange ||
        new Date(resolvedSymptomDate) > endRange
      ) {
        resolvedSymptomDate = today
        if (options.sms.enforceSymptomDateRangeError) {
          throw new BadRequest(
            `Onset date must be between ${format(
              startRange,
              'yyyy-MM-dd'
            )} and ${format(endRange, 'yyyy-MM-dd')}`
          )
        }
      }
    }
    ...
    if (options.issueProxy.url !== '') {
      ...
      const { code, longCode } = await generateVerificationCode(
        request.log,
        offsetSymptomDate,
        resolvedTestType,
        parsedMobile,
