...
async function notify(server, options, done) {
  ...
  // TODO: Is there a better way to achieve this in Fastify?
  const handler = async request => {
    await request.authenticate('push')
    return notifyPositive(request)
  }

  const notifyPositive = async request => {
    const {
      jobId,
      mobile,
      phone,
  ...
  module.exports.notifyPositive = notifyPositive
