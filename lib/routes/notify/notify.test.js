...
describe('notify via sqs', () => {
  ...
  it('should return invalid request due to invalid mobile', async () => {
    const data = { id: '', type: '' }
    const token = jwt.sign(data, options.jwt.secret)
    const mobile = '+353111111111'
    const testType = 'confirmed'
    const onsetDate = format(new Date(), 'yyyy-MM-dd')
    const testDate = format(new Date(), 'yyyy-MM-dd')
    const jobId = '123456'

    const mockSelect = jest
      .fn()
      .mockResolvedValue({ rowCount: 1, rows: [data] })
  ...
  it('should return invalid request due to onset date outside range', async () => {
    const data = { id: '', type: '' }
    const token = jwt.sign(data, options.jwt.secret)
    const mobile = null
    const testType = 'confirmed'
    const onsetDate = format(new Date('2020-01-01'), 'yyyy-MM-dd')
    const testDate = format(new Date(), 'yyyy-MM-dd')
    const jobId = '123456'
  ...
  it('should create a verification code and send and schedule follow ups', async () => {
    const data = { id: '', type: '' }
    const token = jwt.sign(data, options.jwt.secret)
    const mobile = '+353111111111'
    const testType = 'confirmed'
    ...
    options.sms.schedulingEnabled = true
    options.security.encryptKey = 'k4h76mycc9xkmiUQp6swXlQyRsk1YYgB'
